 ...
 * trigger phrase → list of actions (same keys as AssistantCommand.action)
 * Example: "movie mode" → [set_volume:mute, open_app:Netflix]
 */
class MacroStore(context: Context) {
    ...
    data class MacroStep(val action: String, val target: String? = null, val message: String? = null)
